        ...
        if (perms.has(sender, permission)) {
            if (!isPlayerOnly || (isPlayerOnly && sender instanceof Player)) {
                try {
                    ...
                    sender.sendMessage(ChatColor.RED + "Invalid argument '" + e.getInvalidValue() + "'");
                } catch (InvalidArgumentCountException e) {
                    sender.sendMessage(ChatColor.RED + "Invalid argument count.");
                } catch (Exception e) {
                    sender.sendMessage(ChatColor.RED + "An exception occured while executing command. See console for more details.");
                    e.printStackTrace();
                }
            } else {
